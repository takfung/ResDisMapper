...
#'  a grid of cells defining the landscape containing the individuals, and then creates a raster map of resistance. Cells
#'  are displayed only if they have resistances that are statistically different from 0. Red and green contour lines are 
#'  drawn on the map, delineating genetic barriers and corridors, respectively. For further descriptions of the meaning of 
#'  resistances that are statistically different from 0, a genetic barrier and a genetic corridor, please refer to the description 
#'  of the function \code{rdm_resistance}.
...
rdm_mapper <- function(F.df, Geo_raw, r_size = 5, p_signf = 0.05, p_size = 2){
  ...
  F.df2 <- subset(F.df,sign.check==1)

  ggplot2::ggplot(F.df2, ggplot2::aes(x = x, y = y, colour=resistance)) + 
    ...
    ggplot2::geom_point(data = sample.points, cex = p_size, shape = 21, color = "black", fill = "black",stroke = 2)
