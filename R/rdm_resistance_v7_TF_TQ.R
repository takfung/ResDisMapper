#' @name rdm_resistance
#' @title Produces environmental resistance to dispersal over a landscape, for individuals in a population
#'
...
#' @references Tang, Q., Fung, T., Rheindt, F.E. (In review). ResDisMapper: An R package for fine-scale mapping of environmental resistance to dispersal.
#' @examples
#' IBD_res <- rdm_IBD("files/genotypes.gen", "files/coordinates.txt")
#' Res_SLDF <- rdm_residuals(IBD_res, "files/coordinates.txt")
#' rdm_resistance(Res_SLDF)
#' @export
#' @importFrom raster raster extent projection rasterize
#' @importFrom Rmisc CI

rdm_resistance <- function(Res_SLDF, nrows = 30, ncols = 30, conf_intervals = 0.95, random_rep = 1000, outputfile = 'resistance_map.csv'){
